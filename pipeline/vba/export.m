...
% Load BMA results
bma = load(fullfile(paths.bma_dir, 'bma_results.mat'));
...
writetable(table(subject_ids, 'VariableNames', {'subject'}), ...
    fullfile(paths.data_dir, 'subject_ids.csv'));
...
writetable(array2table(coefs, ...
    ...
    fullfile(paths.vba_dir, 'coefficients.csv'), 'WriteRowNames', true);
...
save(fullfile(paths.vba_dir, 'vba_posteriors.mat'), 'mu_all', 'sigma_all', 'subject_ids');
...
writematrix(predictions, fullfile(paths.vba_dir, 'predictions.csv'));
writematrix(decisions, fullfile(paths.vba_dir, 'decisions.csv'));
...
writetable(array2table( ...
    [R2_bma_avg, acc_bma_avg, bacc_bma_avg, log_evidence], ...
    'VariableNames', {'R2', 'accuracy', 'balanced_accuracy', 'log_evidence'}, ...
    'RowNames', subject_ids), ...
    fullfile(paths.vba_dir, 'fit_metrics.csv'), 'WriteRowNames', true);

% Free energy matrix
writematrix(F_all, fullfile(paths.vba_dir, 'free_energy_matrix.csv'));
