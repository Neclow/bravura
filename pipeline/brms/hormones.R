...
VAR_CONFIG <- list(
  TotalCort              = list(int_mean = 0.8, int_sd = 0.5, b_sd = 0.5, sigma_scale = 0.5),
  StressChange_corrected = list(int_mean = 0.0, int_sd = 0.1, b_sd = 0.1, sigma_scale = 0.1),
  Testo_mean             = list(int_mean = 145, int_sd = 60,  b_sd = 60,  sigma_scale = 60),
  TC_ratio               = list(int_mean = 0.1, int_sd = 0.1, b_sd = 0.1, sigma_scale = 0.1)
)
...
fit_one <- function(var, cfg) {
  ...
  priors <- c(
    prior_string(paste0("normal(", cfg$int_mean, ", ", cfg$int_sd, ")"), class = "Intercept"),
    prior_string(paste0("normal(0, ", cfg$b_sd, ")"), class = "b"),
    prior_string(paste0("student_t(3, 0, ", cfg$sigma_scale, ")"), class = "sigma")
  )

  fit <- fit_or_load(
    paste0("fit_", var), out_dir,
    formula = formula, data = df, family = student(), prior = priors,
  ...
  fit_prior <- fit_or_load(
    paste0("fit_", var, "_prior"), out_dir,
    formula = formula, data = df, family = student(), prior = priors,
    sample_prior = "only", chains = CHAINS, cores = CORES,
    iter = 12000, warmup = 2000, seed = SEED, overwrite = OVERWRITE
  )
  ...
  print(bf_results[, c("contrast", "estimate", "Q2.5", "Q97.5", "BF10", "excl_zero")],
        digits = 3)
  ...
  newdata <- expand.grid(Cluster = levels(df$Cluster), Condition = levels(df$Condition))
  epred_long <- newdata %>%
  ...
  write.csv(epred_long[, c("Cluster", var, ".draw")],
            file.path(out_dir, "posterior_epred.csv"), row.names = FALSE)
  ...
  save_diagnostics(fit, paste("student-t", var, "~ Cluster + Condition"),
                   out_dir, prior_fit = fit_prior)
  cat("\nDone:", var, "->", out_dir, "\n")
}
