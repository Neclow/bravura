function [secs, keyCode, deltaSecs] = altKbWait(deviceNumber, forWhat, untilTime,currentTime)
...
% 0.1 msec of scan interval
yieldInterval = 0.0001;
...
secs = -inf;
while (secs-currentTime) < untilTime
    [isDown, secs, keyCode, deltaSecs] = KbCheck(deviceNumber);
    if (isDown == ~forWhat) || ((secs-currentTime) >= untilTime)
        return;
    end
...
%     secs = WaitSecs('YieldSecs', yieldInterval);
    secs = WaitSecs('YieldSecs', yieldInterval); % removing the YieldSecs option makes this use one core at almost 100%. precision morethan doubles.
end
